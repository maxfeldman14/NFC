...
Using human participants, measure the overhead imposed by warnings/approvals presented by our proposed system. As the majority of our study is focused on Android phones, we would provide each user with an Android phone, inform them of what is expected, and have them swipe tags to emulate the particular test cases which have been explained below in more detail.

The overall structure of the experiment is broken down into four main parts. First, we will have users attempt to swipe tags, but present them with no access control approval. These tags will not be malicious. In the second phase, random users will receive malicious tags, again with no access control. We will ask the users to identify what tags were malicious or fake at the end of the experiment in order to avoid priming. Next, we introduce access control and explain the purpose. Again, we will randomly give users malicious tags, and this time the user has the ability to deny or accept with the access control enabled.  
...
Goal:
Test obtrusiveness and effectiveness of access controls in a realistic setting
Test cases:
Smart poster
Grocery store shopping 
Buy a bus ticket with SMS payment
Examine:
Malicious/non-malicious tags
Presence/absence of access control
* important issues and how to deal with them
