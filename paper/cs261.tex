...
\textbf{Establishing secure channels.}
Kortvedt\cite{kortvedt2009} presents a view of NFC vulnerabilities and privacy concerns which remains largely unchanged since the paper's publication in 2009.
He proposes a framework which attempts to provide solutions for establishing a secure channel with mutual authentication via NFC, but it has not gained widespread adoption.
...
\TODO{Cite the standard for ndef signatures}
Signature Record Types, in which signatures could be placed on NDEF records, was standardized to deal with threats caused by malicious tags including spoofing, phishing and denial of service.
The standard now focuses on being able to trust the data through a central authority which distributes and validates public keys\cite{rosati2011}.
However, several vulnerabilities arise with these Signature Record Types due to certain implementation details including different parties being authenticated in partial signatures within the same NDEF message and third party entities having the ability to give trust to other arbitrary entities through certificate chains\cite{roland2011}. 
...
We specify the goals and non-goals of our system and then outline the approach in detail.
\subsection{Goals}
