...
This allows for useful functionality and fast, responsive applications, but it also raises concerns regarding the input of untrusted NFC data.
...
If this application were to be fed a malicious NFC input specifying a tolled phone number, then the user's phone could make a call which charges the user's cellular account, all without the user's knowledge.
...
Additionally, there is no standard means of determining authorization after NFC data is authenticated \cite{roland2011}.
...
We describe the operations present in this system, and how the OS both handles marshalling NFC data after verifying access policies and the utilities provided to the user for access control management.
...
In the case that more than one application can handle the type of data contained in the tag, the phone will open a prompt for the user to choose among the several applications; however, if there is only one application, the phone will automatically launch this application and process the tag, \emph{without user interaction}.
...
Miller's work provides a wide analysis of the NFC attack surface, but our research will seek to provide exploration of application-level vulnerabilities.
...
\begin{table}
  ...
  \hline
  Tag replaced & Tag from trusted organization is replaced with a signed malicious tag. & User is warned when encountering a tag created by a previously unencountered author. User is prompted to accept or reject the public key of the tag. User should be suspicious of untrusted key from previously trusted organization. \\
  \hline
...
We built a proof-of-concept application which uses this access control API in order to provide security for reading NFC tags.\footnote{Unfortunately, we had to return the phone we were using for testing, which has currently halted our development.}
...
Additionally, as we allow finer-grained access to these tags, if the advertising company's prerogative was to instead call a tolled phone number that they provide on the signed tag, our system would prevent the malicious tag writer's attack. 
In this case, the user would have only allowed the movie poster access to certain permissions, not including the \texttt{CALL\_PHONE} permission.
...
To ensure backwards compatibility, unsigned NFC tags should not cause the system to break.
...
This behavior would likely result in users being trained to ignore warnings.
