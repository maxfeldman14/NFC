...
Near Field Communication (NFC) is a form of contactless communication which allows devices to transfer data over radio.
It is similar to RFID, but with a much shorter range (about 10 cm).
NFC has existed for some time, but only recently has begun being deployed in Android phones (along with an API for developers).
...
We also keep in mind the importance of usability, especially in applications which must perform quickly and smoothly (as is the case with NFC applications), and present a user study which can effectively evaluate the tradeoffs between secure access control and usability.
