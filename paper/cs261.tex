...
\sout{A capability in this case refers to the ability to access a system resource, such as to call a phone or open a URL.}
\TODO{A capability in this case refers to the ability for an NFC-enabled application to automatically launch when a tag of a specific \textit{type} and containing data signed with a specific \textit{key}.}
\TODO{Since the choice of application to launch is determined by the type of a tag, and because the behavior of an application on a specific type of tag is usually deterministic, we can authorize application behavior based on type of data in this way.}
\TODO{For example, when a user swipes a tag containing a URL, the web browser application will automatically open the URL.}
\TODO{If we authorize only URL tags created with a specific key, then we can prevent the automatic opening of URLs of untrusted tag.}
