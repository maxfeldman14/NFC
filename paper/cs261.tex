...
\begin{figure}[h!]
	...
	\caption[Caption for LOF]%
		{NDEF Message Format. \\{\scriptsize\url{adapted from http://www.developer.nokia.com/Community/Wiki/Understanding_NFC_Data_Exchange_Format_(NDEF)_messages}}}
...
%Performance. We must consider how our designs constrains the performance of NFC.
%The efficiency of checking our keys? Discuss the upper bound, the number of bits we add to the record with signatures and the fact that we are hashing the key and simply checking if it’s in the ACL with the specific permission. While we don’t have measurements of efficiency, our design does increase the difficulty and time to read a tag since it potentially requires the user to verify the tag at least once.
